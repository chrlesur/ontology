...
func (p *DOCXParser) Parse(path string) ([]byte, error) {
    logger.Debug(i18n.ParseStarted, "DOCX", path)
    
    reader, err := zip.OpenReader(path)
    if err != nil {
        logger.Error(i18n.ParseFailed, "DOCX", path, err)
        return nil, err
    }
    defer reader.Close()
    ...
    logger.Info(i18n.ParseCompleted, "DOCX", path)
...
func (p *DOCXParser) extractMetadata(reader *zip.ReadCloser) {
    for _, file := range reader.File {
        if file.Name == "docProps/core.xml" {
            rc, err := file.Open()
            if err != nil {
                logger.Warning(i18n.MetadataExtractionFailed, "DOCX", err)
                return
            }
            defer rc.Close()
